...
async function loadOverview(){
  ...
  $('overviewStatus').innerHTML=`<div><small>LIVE SOURCE</small><strong>${esc(st)}</strong></div><div><small>OPEN OPPORTUNITIES</small><strong>${esc(s.open_opportunities??0)}</strong></div><p>${o.opportunity_id?'A BEN opportunity is active. The panels below show how it was qualified and how the move is developing.':'No BEN opportunity is currently open. The engine is still evaluating BEN EXIT events against the tested M15, M30 and H1 rules.'}</p>`;
  $('overviewMetrics').innerHTML=[metric('BEN Events Today',p.ben_events_today??0,'Raw BEN EXIT events observed'),metric('Qualified Signals Today',p.qualified_signals_today??0,'Signals that passed a tested rule',p.qualified_signals_today?'good':'info'),metric('Open Opportunities',s.open_opportunities??0,'Unique live opportunities',s.open_opportunities?'good':'info'),metric('Source Lag',`${n(s.source_lag_bars??0,0)} bars`,`${n(s.source_lag_minutes??0,0)} minutes`,s.source_lag_bars?'bad':'good')].join('');
...
async function loadJourney(){
  ...
  $('journeyDetails').innerHTML=[detail('Opportunity ID',esc(h.opportunity_id)),detail('Scope',badge(p.scope,'info')),detail('Start',esc(t(h.start_time))),detail('End / Last Signal',esc(t(h.end_time||h.last_signal_time))),detail('First TF',badge(h.first_tf,'info')),detail('Timeframes',esc(h.timeframes)),detail('Start Price',esc(n(h.first_price||h.start_price,2))),detail('Close Reason',esc(h.close_reason||'Open / horizon not reached'))].join('');
