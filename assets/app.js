...
async function loadHealth(){
  ...
  $('healthStatus').className=`status-band ${kind(s)}`;$('healthStatus').innerHTML=`<div><small>OVERALL STATUS</small><strong>${esc(s)}</strong></div><div><small>LIVE SOURCE AS OF</small><strong>${esc(t(l.live_data_as_of))}</strong></div><div><small>BEN COMPUTE AS OF</small><strong>${esc(t(c.last_market_close_time))}</strong></div><p>${esc(healthMessage(s))}</p>`;
